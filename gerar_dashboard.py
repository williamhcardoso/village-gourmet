...
function initCompras() {
  const despTotal = EXTRATO.filter(e=>e.type==='e').reduce((s,e)=>s+e.value,0);
  document.getElementById('cards-comp').innerHTML = [
    {label:'Fornecedores',       value:FORNEC.length,      sub:'Cadastrados',       cls:'blue',   icon:'&#127981;'},
    {label:'Despesas no Período',value:fmt(despTotal),      sub:'01 a 19/03/2026',   cls:'red',    icon:'&#128176;'},
    {label:'Média por Dia',      value:fmt(despTotal/19),   sub:'19 dias coletados', cls:'yellow', icon:'&#128197;'},
  ].map(d=>`<div class="card ${d.cls}"><div class="card-icon">${d.icon}</div><div class="card-label">${d.label}</div><div class="card-value">${d.value}</div><div class="card-sub">${d.sub}</div></div>`).join('');

  document.getElementById('lista-fornec').innerHTML = FORNEC.length===0
    ? '<p style="color:var(--text2);font-size:.82rem">Nenhum fornecedor cadastrado.</p>'
    : FORNEC.map(f=>`<div class="rank-item">
        <div class="rank-num">&#127981;</div>
        <div>
          <div style="font-size:.85rem;font-weight:500">${f.nome_fantasia||f.razao_nome||'-'}</div>
          <div style="font-size:.75rem;color:var(--text2)">${f.cnpj_cpf||''}</div>
        </div>
      </div>`).join('');

  const catDesp = {};
  EXTRATO.filter(e=>e.type==='e').forEach(e=>{
    const cat = e.category_name||'Outros';
    catDesp[cat] = (catDesp[cat]||0)+e.value;
  });
  const catArr = Object.entries(catDesp).sort((a,b)=>b[1]-a[1]);
  const maxD = catArr[0]?.[1]||1;
  document.getElementById('desp-categoria').innerHTML = catArr.length===0
    ? '<p style="color:var(--text2);font-size:.82rem">Sem despesas registradas.</p>'
    : catArr.map(([cat,val])=>`
      <div style="margin-bottom:9px">
        <div style="display:flex;justify-content:space-between;font-size:.8rem;margin-bottom:3px">
          <span>${cat}</span><span style="color:var(--red)">${fmt(val)}</span>
        </div>
        <div class="prog-wrap"><div class="prog-bar" style="width:${(val/maxD*100).toFixed(0)}%;background:var(--red)"></div></div>
      </div>`).join('');
